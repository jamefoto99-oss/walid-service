...
export async function getReferenceData(): Promise<ReferenceData> {
  ...
  const [
    ...
    supabase.from("parts").select("id,part_code,name,quantity_on_hand").is("deleted_at", null).order("created_at", { ascending: false }),
  ...
  return {
    ...
    parts: (parts.data ?? []).map((row) => option(`${row.part_code} ${row.name} (เหลือ ${row.quantity_on_hand})`, row.id)),
